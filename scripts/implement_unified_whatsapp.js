...
console.log('Implementing Unified Single-Tab WhatsApp Architecture...');

// 1. Update waDispatchConsole modal design
const newWaDispatchModal = `
      ...
      if (state.activeModal === 'waDispatchConsole' && state.modalWaQueue && state.modalWaQueue.length > 0) {
        ...
        const encodedMsg = encodeURIComponent(currentItem.msg);
            ...
            <div class="bg-white border border-slate-200 text-slate-900 rounded-3xl max-w-2xl w-full p-6 sm:p-8 space-y-5 shadow-2xl relative text-left animate-in fade-in zoom-in duration-200">
              ...
              <div class="space-y-1.5">
                <div class="flex justify-between items-center text-xs font-bold text-slate-700">
                  ...
                  <button type="button" onclick="navigator.clipboard.writeText(\`\${currentItem.msg.replace(/\\\`/g, '\\\\\\\\\`').replace(/\\\\/g, '\\\\\\\\\\\\\\\\')}\`); alert('📋 Message copied to clipboard!');" class="text-indigo-600 hover:text-indigo-800 text-xs font-extrabold flex items-center space-x-1">
...
// Replace existing waDispatchConsole block in index.html
html = html.replace(/\/\/ MODAL: IN-APP SEQUENTIAL WHATSAPP DISPATCH CONSOLE[\s\S]*?if \(state\.activeModal === 'waGatewayConfig'\)/, newWaDispatchModal + '\n      if (state.activeModal === \'waGatewayConfig\')');

// 2. Add openSingleWhatsAppTab, dispatchCurrentQueueItemAndAdvance, copyAllBroadcastPhoneNumbers, exportBroadcastQueueToCsv helper functions
const whatsappEngineCode = `
...
// Replace existing openWhatsAppDirect function in index.html
html = html.replace(/\/\/ --- SINGLE FLOATING SIDE-WINDOW WHATSAPP DISPATCHER[\s\S]*?function openWaGatewayConfigModal\(\)/, whatsappEngineCode + '\n    function openWaGatewayConfigModal()');

// 3. Update individual 1-Click WhatsApp buttons in table to directly call openSingleWhatsAppTab
const oldRowButton = `                              <button 
                                data-wa-phone="\${formattedPhone}"
                                data-wa-msg="\${fullMsg.replace(/"/g, '&quot;')}"
                                data-wa-name="\${s.name}"
                                onclick="startSequentialWhatsAppDispatch(\${idx})" 
                                class="inline-flex items-center space-x-1 bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-xl font-bold text-[11px] shadow-2xs active:scale-95 transition-all">
                                <span>📲 WhatsApp</span>
                              </button>`;

const newRowButton = `                              <button 
                                data-wa-phone="\${formattedPhone}"
                                data-wa-msg="\${fullMsg.replace(/"/g, '&quot;')}"
                                data-wa-name="\${s.name}"
                                onclick="openSingleWhatsAppTab('\${formattedPhone}', \`\${fullMsg.replace(/\\\`/g, '\\\\\\\\\`').replace(/\\\\/g, '\\\\\\\\\\\\\\\\').replace(/\\\$/g, '\\\\\\\\$')}\`)" 
                                class="inline-flex items-center space-x-1 bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-xl font-bold text-[11px] shadow-2xs active:scale-95 transition-all" title="Opens or updates 1 single WhatsApp tab">
                                <span>📲 WhatsApp (1 Tab)</span>
                              </button>`;

html = html.replace(oldRowButton, newRowButton);

// 4. Update executeMassBroadcastModal to open waDispatchConsole queue runner immediately
html = html.replace(
  "state.modalBroadcastData = newLog;\n      state.activeModal = 'broadcastProgress';",
  "state.modalBroadcastData = newLog;\n      startSequentialWhatsAppDispatch();"
);

// 5. Update startSequentialWhatsAppDispatch to populate rich student data in queue
const newStartDispatch = `
    function startSequentialWhatsAppDispatch(startIndex = 0) {
      ...
      const msgCategory = state.broadcastCategory || 'absence';
      const attachedPhoto = state.broadcastAttachedPhoto || '';
      const defaultMsgText = state.broadcastMessageText || 'Dear Parent, this is an official circular from ST. VENUS HIGH SCHOOL.';
      ...
      if (recipientList.length === 0) {
        alert('⚠️ No parent records found matching your target audience filter (' + targetAudience + ').');
        return;
      }

      const queue = recipientList.map(s => {
...
html = html.replace(/function startSequentialWhatsAppDispatch\(startIndex = 0\) \{[\s\S]*?state\.activeModal = 'waDispatchConsole';\s*render\(\);\s*\}/, newStartDispatch);

fs.writeFileSync(htmlPath, html, 'utf-8');
console.log('✅ Single-Tab WhatsApp Architecture injected into index.html.');
...
if (allOk) {
  console.log('\n🎉 ALL SCRIPTS SYNTACTICALLY VERIFIED AND READY TO DEPLOY!\n');
}
